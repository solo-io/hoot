...
func (s *processorService) Process(stream extproc.ExternalProcessor_ProcessServer) error {
	...
	logger.Debug("First message: ", msg)
	...
	case "/blockLocalHost":
		return processBlockLocalHost(stream, msg)
	case "/dynamicMetadata":
		return processDynamicMetadata(stream, headers)
	case "/notfound":
...
func processBlockLocalHost(stream extproc.ExternalProcessor_ProcessServer, msg *extproc.ProcessingRequest) error {
	attsMsg := msg.GetAttributes()
	if attsMsg == nil || len(attsMsg) == 0 {
		sendImmediateResponse(stream, http.StatusBadGateway, "expected attributes", "")
	}
	atts, ok := attsMsg["envoy.filters.http.ext_proc"]
	if !ok {
		sendImmediateResponse(stream, http.StatusBadGateway, "expected attributes message to have ext_proc key", "")
	}

	addr, ok := atts.Fields["source.address"]
	if !ok {
		sendImmediateResponse(stream, http.StatusBadGateway, "expected attributes to contain source.address", "")
	}

	if firstPart := strings.Split(addr.GetStringValue(), ".")[0]; firstPart == "127" || firstPart == "localhost" {
		logger.Infof("blocking request because address begins with %v", firstPart)
		sendImmediateResponse(stream, http.StatusForbidden, "requests may not be made from localhost", "")
	}
	return nil
}

func processDynamicMetadata(stream extproc.ExternalProcessor_ProcessServer, headers *extproc.HttpHeaders) error {
	metadataStr := getHeaderValue(headers.GetHeaders(), "x-set-metadata")

	if metadataStr == "" {
		sendImmediateResponse(stream, http.StatusBadRequest, "no x-set-metadata header", "")
	}

	// Send back a response with the extracted dynamic metadata
	return stream.Send(&extproc.ProcessingResponse{
		Response: &extproc.ProcessingResponse_RequestHeaders{
			RequestHeaders: &extproc.HeadersResponse{
				Response: &extproc.CommonResponse{},
